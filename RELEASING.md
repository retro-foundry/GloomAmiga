...
This repository releases Windows and DOS builds through GitHub Actions. Pushing a version tag matching `v*.*.*`, for example `v0.1.0`, builds clean Windows and DOS archives from the tagged source, runs the available self-tests, creates SHA256 checksums, records artifact attestations, and publishes the files to a GitHub Release using `GITHUB_TOKEN`.
